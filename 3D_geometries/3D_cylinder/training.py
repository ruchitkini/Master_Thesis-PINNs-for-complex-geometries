...
data = dde.data.PDE(geom, pde, [s22_top_bc, s21_top_bc, s23_top_bc,
                                s1_curve_bc, s2_curve_bc, s3_curve_bc],
                    num_domain=25000,
                    num_boundary=2000,
                    num_test=20000)
...
checkpointer = dde.callbacks.ModelCheckpoint("model_random/model", verbose=1, save_better_only=True)

model.compile("adam", lr=0.001, decay=("inverse time", 20000, 0.912), loss_weights=[1, 1, 1, 1, 1, 1, 1, 1, 1, 100, 100, 100, 100, 100, 100])
losshistory, train_state = model.train(display_every=1000, iterations=2500_000, callbacks=[checkpointer])
