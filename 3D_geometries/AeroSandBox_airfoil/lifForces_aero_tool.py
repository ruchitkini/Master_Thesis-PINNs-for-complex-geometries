...
wing_airfoil = asb.Airfoil("naca0006")
...
class VortexLatticeMethod(ExplicitAnalysis):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        face_centers = 0.25 * (front_left_vertices + back_left_vertices + front_right_vertices + back_right_vertices)
        ...
        plotter = pv.Plotter()
        plotter.add_mesh(surface, style='surface', show_edges=True, color='white')
        # plotter.add_points(cloud, color="green", point_size=4)

        cloud = pv.PolyData(collocation_points)
        plotter.add_points(cloud, color="red", point_size=4)
        # plotter.add_arrows(collocation_points, normal_directions, mag=0.01, color='red')  #
        # cloud_l = pv.PolyData(front_right_vertices)
        # plotter.add_points(cloud_l, color="red", point_size=4)
        cloud_r = pv.PolyData(right_vortex_vertices)
        # plotter.add_points(cloud_r, color="green", point_size=4)
        cloud_v = pv.PolyData(vortex_centers)
        plotter.add_points(cloud_v, color="blue", point_size=4)
        cloud_face_center = pv.PolyData(face_centers)
        plotter.add_points(cloud_face_center, color="black", point_size=4)
        plotter.show()
        # ================================================================================================== #
        # ================================================================================================== #
        ...
        # ================================ MY CHANGES IN SOURCE CODE =================================== #
        # ============================================================================================== #
        forces_body = self.op_point.convert_axes(
            forces_geometry[:, 0], forces_geometry[:, 1], forces_geometry[:, 2],
            from_axes="geometry",
            to_axes="body"
        )
        forces_wind = self.op_point.convert_axes(
            forces_body[0], forces_body[1], forces_body[2],
            from_axes="body",
            to_axes="wind"
        )

        Lift_forces = -forces_wind[2]
        L_f = np.sum(Lift_forces)

        pressure_dist = Lift_forces/areas
        pressure_dist_shape = np.zeros((len(pressure_dist), 1))
        pressure_dist_shape[:, 0] = pressure_dist

        df_points = pd.DataFrame(points, columns=['xp', 'yp', 'zp'])
        df_faces = pd.DataFrame(faces, columns=['f1', 'f2', 'f3', 'f4'])
        df_face_cent = pd.DataFrame(face_centers, columns=['xf', 'yf', 'zf'])
        df_pressure = pd.DataFrame(pressure_dist_shape, columns=['pressure'])
        df_data = pd.concat([df_points, df_faces, df_face_cent, df_pressure], axis=1)
        # df_data.to_csv("pressure_distribution.csv", index=False)
        df_data.to_csv("pressure_distribution_coarse.csv", index=False)


        plotter2 = pv.Plotter()
        plotter2.add_mesh(surface, style='surface', color='white')
        plotter2.add_points(cloud_v, scalars=Lift_forces, point_size=6)
        plotter2.show()

        surface_face = cloud_face_center.delaunay_2d()
        plotter3 = pv.Plotter()
        plotter3.add_mesh(surface_face, style='surface', scalars=pressure_dist, color='white')
        # plotter3.add_points(cloud_face_center, scalars=pressure_dist, point_size=6)
        plotter3.show()
...
# ---------- Call the vortex lattice method ---------- #
