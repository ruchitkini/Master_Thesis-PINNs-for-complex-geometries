...
PINN Model for linear elasticity analysis on 3D airfoil - NACA0006
...
coords_boundary_bottom[:, 0], coords_boundary_bottom[:, 1], coords_boundary_bottom[:,
                                                            2] = x_traction, y_traction, z_traction
...
def generate_data(num):
    data = pd.read_csv("results/abaqus_result.csv")
...
data = dde.data.PDE(geom, pde, [s11_end_bc, s33_free_bc, s12_end_bc, s23_free_bc, s13_end_bc, s13_free_bc,
                                observe_ux, observe_uy, observe_uz, observe_sxx, observe_syy, observe_szz, observe_sxy,
                                observe_syz, observe_sxz],
...
model.compile("adam", lr=0.001, decay=("inverse time", 50000, 0.95),
              loss_weights=[0.01, 0.01, 0.01, 0.000001, 0.000001, 0.000001, 0.000001, 0.000001, 0.000001,
                            100, 100, 100, 100, 100, 100,
                            10, 10, 10, 1, 1, 1, 1, 1, 1])
model.compile("L-BFGS")

# Restore the saved model with the smallest training loss
model.restore(f"model/model-656000.pt", verbose=1)
