...
checkpointer = dde.callbacks.ModelCheckpoint("model_roller/model", verbose=1, save_better_only=True)

model.compile("adam", lr=0.001, loss_weights=[1, 1, 1, 1, 1, 10, 10, 10, 10, 10, 10])
model.train(iterations=15000, display_every=1000, callbacks=[checkpointer])
dde.optimizers.config.set_LBFGS_options(maxiter=25_000)
