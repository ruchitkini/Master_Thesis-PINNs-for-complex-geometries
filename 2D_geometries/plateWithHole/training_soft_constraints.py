...
solid mechanics "
Link: https://doi.org/10.1007/s00466-022-02252-0

BC:     ux = 0      (left boundary),    uy = 0      (bottom boundary), --> implimented as soft contraints
        sx = 1      (right boundary) --> implimented as soft contraints
        Traction at remaining boundaries need to be explicitly mentioned: s.n = 0

Optimiser: ADAM (5000 iterations) + L-BFGS (45000 iterations)
"""
...
# ---------- Define the functions for traction conditions ---------- #
...
model.compile("adam", lr=0.001, loss_weights=[1, 1, 0.1, 0.1, 0.1, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10])

# ---------- Save the model during training ---------- #
checkpointer = dde.callbacks.ModelCheckpoint("model3/model", verbose=1, save_better_only=True)
